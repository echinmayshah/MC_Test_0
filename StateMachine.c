...
*
* Description: This file implements the motor control state machine.
*
...
//vres_0_motor_0_FAST_IRQn
//c start
uint8_t mTrack = 0;	
uint8_t TrackArr[16] = {0};
uint32_t 	Track_0 = 0, Track_1 = 0, Track_2 = 0, Track_3 = 0, Track_4 = 0, Track_5 = 0, \
			Track_6 = 0, Track_7 = 0, Track_8 = 0, Track_9 = 0, Track_10 = 0, \
			Track_11 = 0, Track_12 = 0, Track_13 = 0;
uint8_t trace_check = 0;
//c end
...
static void CommonISR0()
{
    ...
    SENSOR_IFACE_RunISR0();		//c measure ADC Current, Clark Transform to converted into Alpha-Beta
    FAULT_PROTECT_RunISR0();	//c measure relevant value from feedback current for Fault Check
}
RAMFUNC_END
...
void STATE_MACHINE_ResetAllModules()	//c set ADC_filter co-efficients, all fault variables/flags reset, set SVM as a modulation & modulation_index = 0, all pid, phase-advance, six-pulse injection var/para reset, FeedbackISR0Wrap = OBS_RunISR0;
{
    SENSOR_IFACE_Init();	//c set ADC_filter co-efficients
    FAULT_PROTECT_Init();	//c all fault variables/flags reset - bit complicated to understand
    OBS_Init();				//c blank call
    CTRL_FILTS_Init();		//c BIQUAD_PoleZero_Init - no need to deep-into
    SPEED_CTRL_Init();		//c init speed-pid para/variables with initialized val
    VOLT_MOD_Init();		//c set SVM as a modulation & modulation_index = 0
#if defined(CTRL_METHOD_RFO)
    PHASE_ADV_Init();		//c phase-advance para/var init
#elif defined(CTRL_METHOD_SFO)
    FLUX_CTRL_Init(1.0f);
    TRQ_Init();
    DELTA_CTRL_Init();
#endif
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_SFO)
    SIX_PULSE_INJ_Init();	//c six-pulse inj var/para init
    HIGH_FREQ_INJ_Init();	//c black call
    PROFILER_Init();		//c black call
#endif
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    HALL_SENSOR_Init();		//c NA
    INC_ENCODER_Init();		//c black call
    CURRENT_CTRL_Init(1.0f);	//c id/iq_pid para/var init
#endif
    // Reset CommonISR modules first:
    SENSOR_IFACE_Reset();	//c all filter val reset
    FAULT_PROTECT_Reset();	//c all fault reset
...
void STATE_MACHINE_ResetVariable(void)	//c BIQUAD, ADC_Filter, Fault struct, PIDs, SpeedRef, d_q comp, pi/2, six_pulse_inj.state, etc. reset
{
    ELEC_t w0 = Elec_Zero;

    CTRL_FILTS_Reset();		//c BIQUAD reset
    SENSOR_IFACE_Reset();	//c all ADC filter val reset
    FAULT_PROTECT_Reset();	//c all fault reset
    SPEED_CTRL_Reset();		//c speed-pid reset
    VOLT_CTRL_Reset();		//c SpeedRef reset - mech speed
    CTRL_ResetWcmdInt(w0);	//c SpeedRef with direction reset - elect speed

    vars.v_qd_r_cmd.d = 0.0f;	//c d_q components variable reset
    vars.v_qd_r_cmd.q = 0.0f;

#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_SFO)
    ELEC_t th0 = { PI_OVER_TWO };	//c pi/2=(3.14/2=1.57)
    if (params.sys.fb.mode == Sensorless)
    {
       ...
       OBS_Reset(&la_ab_lead, &w0, &th0);	//c blank call
    }
    SIX_PULSE_INJ_Reset();		//c six_pulse_inj.state = Not_Started;
    TRQ_Reset();				//c vars.T_est_filt = 0.0f; - need to find its usage
    HIGH_FREQ_INJ_Reset(Elec_Zero, ctrl.six_pulse_inj.th_r_est);	//c black call
#endif
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    if (params.sys.fb.mode == Hall)
    {
    ...
    CURRENT_CTRL_Reset();	//c dq-pid reset
    vars.i_cmd_int = 0.0f;
    ...
    FLUX_WEAKEN_Reset();		//c ctrl.flux_weaken.integ = 0.0f;
    vars.i_qd_r_cmd.d = 0.0f;	//c d_q cmd_n_Fdb values reset
...
static void InitEntry()	//c PWM_HiZ, mcdi_disable, variables init, peripherals & variables reset_then_init, ADC Cali_init, PWM/Timer_init, set ADC_filter co-efficients, all fault variables/flags reset, set SVM as a modulation & modulation_index = 0, all pid, phase-advance, six-pulse injection var/para reset, mcdi_enable & start, ADC start
{
    {
		static uint8_t init = 0;
	...
	hw_fcn.GateDriverEnterHighZ();	//c PWM_UVW HighZ

    if (!sm.vars.init.param_init_done)    // only after booting up
    {
        hw_fcn.StopPeripherals();        // disable all ISRs, PWMs, ADCs, etc. //c simply mcdi_disable @MCU.c

        CommonISR0Wrap = EmptyFcn;
        CommonISR1Wrap = EmptyFcn;
        FeedbackISR0Wrap = OBS_RunISR0;
#if !defined(PC_TEST)
        PARAMS_Init();						//c all parameters/variables initialized in this 
#endif
        hw_fcn.HardwareIfaceInit();        	// all peripherals must stop before re-initializing	//c Chip, ADC_Cali, PWM/Timer Inits - @MCU.c
        STATE_MACHINE_ResetAllModules();	//c set ADC_filter co-efficients, all fault variables/flags reset, set SVM as a modulation & modulation_index = 0, all pid, phase-advance, six-pulse injection var/para reset, FeedbackISR0Wrap = OBS_RunISR0;
        sm.vars.init.param_init_done = true;
        vars.en = true;

        hw_fcn.StartPeripherals();        // enable all ISRs, PWMs, ADCs, etc. //c call to MCU_StartPeripherals(); @ MCU.c - //c mcdi_enable & start, ADC start
    }
    STATE_MACHINE_ResetVariable();	//c BIQUAD, ADC_Filter, Fault struct, PIDs, SpeedRef, d_q comp, pi/2, six_pulse_inj.state, etc. reset
...
RAMFUNC_BEGIN
static void InitISR0()	//c current calibration - as does in ti - Op-amp_null_offset
{
    {
		static uint8_t init = 0;
	...
	if (sm.vars.init.param_init_done)
    {
        ...
        {
            SENSOR_IFACE_OffsetNullISR0();	//c current calibration - as does in ti - Op-amp_null_offset
        }
    }
}
RAMFUNC_END

//c inside void BrakeBootEntry(void)
//c reset - BIQUA, IFdb_UVW(sensed_calibrated_filetered_val), SpeedRef, speed1.Speed
//c GateDriverHiZ
static void BrakeBootEntry()	
{
    ...
    CTRL_FILTS_Reset();					//c BIQUAD reset
    vars.d_uvw_cmd = UVW_Zero;			//c IFdb_UVW - sensed_calibrated_filetered val
    vars.w_cmd_int.elec = 0.0f;			//c SpeedRef
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    vars.i_cmd_int = 0.0f;				//c speed-pid o/p after applied limit
#elif defined(CTRL_METHOD_SFO)
    vars.T_cmd_int = 0.0f;
#endif
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_SFO)
    ...
    hw_fcn.GateDriverExitHighZ();		//c GateDriverHiZ
    hw_fcn.ExitCriticalSection();    	// --------------------
}

RAMFUNC_BEGIN
static void BrakeBootISR0()	//c wait untill pot input for speed i/p above min_thresold val
{
    {
		static uint8_t init = 0;
...
static void BrakeBootISR1()	//c wait untill pot input for speed i/p above min_thresold val
{
...
static void VoltHzOLEntry()		//c align SpeedRef*Dir
{
    {
		static uint8_t init = 0;
	...
	CTRL_ResetWcmdInt(params.ctrl.volt.w_thresh);	//c align speed_n_direction (vars.w_cmd_int.elec = params.ctrl.volt.w_thresh.elec * vars.dir;)
                                                    //c params.ctrl.volt.w_thresh.elec is min speed for open-loop 
    vars.v_qd_r_cmd.d = 0.0f;						//c seems openloop_volt_control dq cmd_val to be reset
    VOLT_CTRL_Reset();	//c? 
    hw_fcn.EnterCriticalSection();    // --------------------
...
static void VoltHzOLExit()
{
	...
	if (params.sys.analog.shunt.type == Single_Shunt)
    {
        VOLT_MOD_EnDisHybMod(En);	//c? 1-shunt specific 
    }
}

//c inside VoltHzOLISR0(void)
//c v_ol_spd_ramp-control
//c inverse-park x'form, SVM
//c filter_n_absolute speed val & derives vars.acc_cmd_int.elec quantity from vectored_v_ol_speed
RAMFUNC_BEGIN
...
{	
   {
	...
	CTRL_UpdateWcmdIntISR0(vars.w_cmd_ext);	//c ramp control for v_ol_spd
    vars.v_qd_r_cmd.q = MAX(params.ctrl.volt.v_min + ABS(vars.w_cmd_int.elec) * params.ctrl.volt.v_to_f_ratio, 0.0f) * vars.dir;	//c q val of V/F for inverse-park x'form
    VOLT_CTRL_RunISR0();		//c inverse-prk x'form
    VOLT_MOD_RunISR0();			//c SVM (used inverse-park o/p)
    CTRL_FILTS_RunAllISR0();	//c filter_n_absolute speed val & derives vars.acc_cmd_int.elec quantity from vectored_v_ol_speed
}
RAMFUNC_END

static void SpeedCLEntry()	//c derive min speed for OL, reset speed-pid & flux weaken
{
	{
		static uint8_t init = 0;
    ...
    if ((sm.current == Align) || (sm.current == Six_Pulse) || (sm.current == High_Freq) || (sm.current == Brake_Boot))
    {
        ...
        CTRL_ResetWcmdInt(w0);	//c align speed with dir 
        SPEED_CTRL_Reset();		//c reset speed-pid
        vars.i_cmd_int = 0.0f;	
        FLUX_WEAKEN_Reset();	//c?
    }
#elif defined(CTRL_METHOD_SFO)
...
RAMFUNC_BEGIN
static void SpeedCLISR0()	//c ramp control, speed-pid, phase-advance, field weaken, park x'form, Id-Iq_PIDs, SVM
{
    {
		static uint8_t init = 0;
	...
	CTRL_UpdateWcmdIntISR0(vars.w_cmd_ext);	//c ramp control
    FeedbackISR0Wrap();	                    			//c empty calls
    switch (params.sys.fb.mode)	            			//c fb.mode = Sensorless 
    {
    default:
    case Sensorless:
        vars.w_final.elec = vars.w_est.elec;			//c speed(Estimated by observer)
        vars.th_r_final.elec = vars.th_r_est.elec;		//c angle/position(Estimated by observer) 
        break;
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    case Hall:
        vars.w_final.elec = vars.w_hall.elec;
    ...
    CTRL_FILTS_RunAllISR0();	//c calc of (1). vars.w_final_filt_abs.elec & (2). vars.acc_cmd_int.elec(acceleration_component)
    SPEED_CTRL_RunISR0();		//c speed-pid alongwith speed inertia 
#if defined(CTRL_METHOD_RFO)
    vars.i_cmd_prot = SAT(-protect.motor.i2t.i_limit, protect.motor.i2t.i_limit, vars.i_cmd_spd);	//OutMax, OutMin on spd-pid_outout(vars.i_cmd_spd)
    vars.i_cmd_int = RateLimit(params.sys.rate_lim.i_cmd * params.sys.samp.ts0, vars.i_cmd_prot, vars.i_cmd_int);	//c OutMax_OutMin on vars.i_cmd_int 
                                                                                                                    //c vars.i_cmd_int = vars.i_cmd_prot;
                                                                                                                    //c above two will cap n limit speed-pid output
    PHASE_ADV_RunISR0();		//c applying phase advance(produce dq component) on speed-pid_capped_n_limit_val(vars.i_cmd_int)
    FLUX_WEAKEN_RunISR0();		//c applying field weakening(produce dq component) on speed-pid_capped_n_limit_val(vars.i_cmd_int)
    CURRENT_CTRL_RunISR0();		//c park x'form, pid-dq & inverse park x'form
    TRQ_RunObsISR0();			//c seems angle estimator
    VOLT_MOD_RunISR0();			//c SVM
#elif defined(CTRL_METHOD_SFO)
...
static void SpeedOLToCLEntry()	//c reset many variables n wait n start stop-watch
{
    {
		static uint8_t init = 0;
...
//c inside void SpeedOLToCLISR0(void)
//c v_ol_spd_ramp-control 
//c park x'form 
//c angle estimation
//c inverse-park x'form, SVM
...
static void SpeedOLToCLISR0()	
{
	...
	CTRL_UpdateWcmdIntISR0(vars.w_cmd_ext);	//c ramp-control
    FeedbackISR0Wrap();									//c seems empty calls
    ELEC_t th_r_trq = { vars.th_r_est.elec };			//c angle-Fdb val by using observer
    PARK_t park_r_trq;									//c sin-cosine
    ParkInit(th_r_trq.elec, &park_r_trq);	//c standard motor-control API
    ParkTransform(&vars.i_ab_fb, &park_r_trq, &vars.i_qd_r_fb);	//c park x'form - standard motor-control API
    TRQ_RunObsISR0();									//c seems angle estimator

    vars.v_qd_r_cmd.q = MAX(params.ctrl.volt.v_min + ABS(vars.w_cmd_int.elec) * params.ctrl.volt.v_to_f_ratio, 0.0f) * vars.dir;
    VOLT_CTRL_RunISR0();		//c inverse-park x'form
    VOLT_MOD_RunISR0();			//c SVM - call to standard-motor-control API
    CTRL_FILTS_RunAllISR0();	//c filter_n_absolute speed val & derives vars.acc_cmd_int.elec quantity from vectored_v_ol_speed
...
//c inside SpeedOLToCLExit(void)
//c reset speed-pid
//c park x'form
//c seems phase_advance
//c spd-pid_gain_set/init/derived
//c reset flux_weaken para/var
//c reset dq-pid
static void SpeedOLToCLExit()	
{
    ...
    ParkTransform(&i_ab_fb, &park_r, &i_qd_r_fb);	//c park x'form

#if defined(CTRL_METHOD_RFO)
    float i_cmd_spd;
    PHASE_ADV_CalcOptIs(&i_qd_r_fb, &i_cmd_spd);	//c? - seems phase_advance
    vars.i_cmd_int = i_cmd_spd * params.ctrl.speed.ol_cl_tr_coeff;
    SPEED_CTRL_IntegBackCalc(vars.i_cmd_int);	//c spd-pid_gain_set/init/derived
    ...
    FLUX_WEAKEN_Reset();	//c reset flux_weaken para/var
    CURRENT_CTRL_Reset();	//c dq-pid reset
...
{
    CommonISR0Wrap();	//c measure ADC Current, Clark Transform to converted into Alpha-Beta
						//c measure relevant value from feedback current for Fault Check

    sm.states[sm.current].RunISR0();

    sm.add_callback.RunISR0();	//c used only for profile - to get/tune motor para
}
RAMFUNC_END

void STATE_MACHINE_RunISR1()
{
	static uint8_t ij = 0, jk = 0, iArr[15] = { 0 }; //c
    CommonISR1Wrap();	//c measure ADC other val - pot, temp, etc
						//c measure relevant value from feedback current for Fault Check

    sm.states[sm.current].RunISR1();

    sm.add_callback.RunISR1();	//c used only for profile - to get/tune motor para
